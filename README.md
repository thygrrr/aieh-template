...
If `GameInput.ui_exit_enabled` is ever turned off (e.g. a misbehaving black
button), the hold-to-quit is disabled too (a stuck-down button would trigger
it by itself); quit by holding a Start button for 3 seconds instead.
...
- `ui_exit` is implemented by the `GameInput` autoload
  ...
  GAME_SPEC.md. Can be disabled via `ui_exit_enabled` (quit then goes
  through the pause overlay, hold Start 3 s) if the black button ever
  misbehaves again.
